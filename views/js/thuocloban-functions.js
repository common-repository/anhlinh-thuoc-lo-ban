...
function pullRightAction () {
    if(trimStart > 0) {
        ...
        var newLi = jQuery('<li>').append(jQuery('<img/>', { src:'./thuoc522.png'+qStr }))
            .append(jQuery('<img/>', { src:'./thuoc429.png'+qStr }))
            .append(jQuery('<img/>', { src:'./thuoc388.png'+qStr }));
...
function loaded() {
    ...
    myScroll = new iScroll('loban-wrapper', {
        ...
        onRefresh: function () {
            ...
            //jQuery('#container-sodo').css({'left': (jQuery('#lobanOuter').width()/2 - 32) + 'px' });
            ...
            getNoiDungHienThi(Math.round((-this.x+jQuery('#lobanOuter').width()/2)/100,1));
        },
...
function getNoiDungHienThi(valcm) {
    console.log("Please upgrade to Pro version show text detail");
}
